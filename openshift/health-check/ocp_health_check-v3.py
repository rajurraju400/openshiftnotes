...
""")
    ...
    hub_commands = [
        ...
        "oc api-resources",
        "oc get node -A -o wide --show-labels",
        "oc get pods -A -o wide --show-labels",
        "oc get ns",
        "oc get all -A -o wide",
        "oc get pv,pvc,storageclass  -A -o wide",
        "oc get configmap -A -o wide",
        "oc get secret  -A -o wide",
        "oc adm top nodes",
        "oc adm top pods -A",
        "oc get po -A",
        ...
        for i in $(oc get nodes -o custom-columns=NAME:.metadata.name --no-headers); do
            echo ">>> Node: $i"
            oc debug node/$i -- bash -c 'chroot /host bash -c "df -kh ; free -g"' || echo "Failed on $i"
            echo "---------------------------------------------"
        done
        """,
    ...
    spoke_commands = [
        ...
        [ -n "$cert" ] && echo "$cert" | base64 -d | openssl x509 -noout -enddate 2>/dev/null | sed 's/notAfter=//' || echo "NO CERT"; \
         ...
         """,
        # Checking for any pod in not ready
        """
        oc get pods --all-namespaces --no-headers | awk '$4 != "Running" && $4 != "Completed" {print $0}'
        """,
        "oc config view",
        "oc cluster-info dump",
        "oc api-resources",
